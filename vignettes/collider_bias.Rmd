...
# read in the data
gwas_clumped_incidence <- data.table::fread(clumped_incidence_path)
...
We can use the `analyse_collider_bias()` function to run all combinations of methods and parameters. Here is the (in-)stability of the b-slope with different p-values (`ip`).
...
corrected_gwas <- apply_collider_correction(gwas_i = gwas_incidence, 
                                            gwas_p = gwas_progression,
                                            merge  = c("CHR"="CHR", "BP"="BP"),
                                            b_correction_factor = -0.6558253,
                                            b_std_err = 0.09785357)
...
p <- ggplot2::ggplot(data    = corrected_gwas,
  ...
  ggplot2::geom_point(mapping = ggplot2::aes(shape = sig_change, 
                                             fill  = adjusted_beta, 
                                             color = sig_change), alpha=0.95, position = position_jitter(seed = 1, width = 0.1)) +
