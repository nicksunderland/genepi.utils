...
#' #' @description
#' #' Standardises a GWAS with appropriate column names and data checks using the following procedure:
#' #' \tabular{rrrrr}{
#' #'   \strong{Step} \tab \strong{Description} \cr
#' #'   Column mapping \tab Define the mapping of column names \cr
#' #' }
...
#'   }
#'
#'   # parse the column mapping to a named list(standard name = old name, ...)
#'   mapping <- get_gwas_mapping(input_format)
#'
#'   # get the gwas from file (if `gwas` is a file path) or object (if `gwas` is a data.frame-like object)
...
#'   gwas <- filter_invalid_values(gwas, filters, remove)
...
#' #' @param gwas a data.table
#' #' @param remove where to remove rows, if FALSE then logical columns for the filters are appended instead
#' #' @return a filtered data.table
#' #' @noRd
#' #'
